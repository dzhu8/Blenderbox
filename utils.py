...
def calculate_camera_distance(
    object_scale: float, fov: float = np.pi / 3.0, screen_fill_percentage: float = 0.75
) -> float:
    """
    Calculate camera distance to position an object to fill a certain percentage of the screen.

    Args:
        object_scale: The scale (largest dimension) of the object
        fov: Field of view angle in radians (default: pi/3 or 60 degrees)
        screen_fill_percentage: Percentage of the screen width/height that the object should fill (0.1 to 1.0)
                                Higher values bring the camera closer to the object.
                                Lower values position the camera farther away.

    Returns:
        The optimal camera distance to achieve the desired screen fill percentage
    """
...
def get_object_dimensions(
    mesh_or_scene: Union[trimesh.Trimesh, trimesh.Scene, str],
) -> Dict[str, Any]:
    """
    ...
    if isinstance(mesh_or_scene, trimesh.Scene):
        ...
        volume = (
            sum(
                [
                    m.volume
                    for m in mesh_or_scene.geometry.values()
                    if hasattr(m, "volume")
                ]
            )
        ...
        surface_area = (
            sum([m.area for m in mesh_or_scene.geometry.values() if hasattr(m, "area")])
            if mesh_or_scene.geometry
            else None
        )
    else:
...
def calculate_light_intensity(
    dimensions: Union[Dict[str, Any], float],
    ...
    Args:
        dimensions: Dictionary containing object dimensions (from get_object_dimensions)
                   or directly a float representing diameter
